...
function viewOffset() { return state.frameStart ?? 0; }
// ── Downsampling ──────────────────────────────────────────────────
function getDisp(rows) {
    if (rows.length <= MAX_PTS)
        return rows;
...
// ── Panel drawing ─────────────────────────────────────────────────
function drawPanel(ctx, s, panelY, plotW) {
    ...
    const toY = (v) => panelY + MT + pH * (1 - (v - lo) / (hi - lo));
    const toX = (i) => ML + (n > 1 ? i / (n - 1) : 0) * plotW;
    ctx.strokeStyle = '#e2e8f0';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(ML, panelY + MT);
    ctx.lineTo(ML + plotW, panelY + MT);
    ctx.stroke();
    ...
    const drawCutoff = (fv) => {
        if (fv === undefined || !isFinite(fv) || fv < lo || fv > hi)
            return;
        const y = toY(fv);
        ctx.save();
...
let brushDragStart = null;
let brushDragEnd = null;
function drawBrush(ctx, totalH) {
    if (!brushDragging || brushDragStart === null || brushDragEnd === null)
        return;
    ...
    ctx.beginPath();
    ctx.moveTo(x1, MT);
    ctx.lineTo(x1, MT + clipH);
    ctx.moveTo(x2, MT);
    ctx.lineTo(x2, MT + clipH);
    ctx.stroke();
}
// ── Canvas cache ──────────────────────────────────────────────────
let cachedW = 0;
...
export function drawTimeline() {
    ...
    if (!state.allRows.length || !state.curCol) {
        hide(card);
        return;
    }
    show(card);
...
export function initTimeline(onFrameChange) {
    const canvas = document.getElementById('timelineCanvas');
    ...
    canvas.addEventListener('mouseup', e => {
        ...
        zoomHistory.push(state.frameStart !== null
