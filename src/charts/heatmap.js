...
export function drawHeatmap(rows) {
    ...
    if (sorted.length > MAX_HEAT_ROWS) {
        ...
        for (let i = 0; i < MAX_HEAT_ROWS; i++) {
            ...
            const agg = { [state.curCol]: mean(bucket.map(r => toNum(r[state.curCol]))) };
